import { baseQuestions, insuranceTypes, quickQuestions } from "./data.js";

const PRIMARY_LIMIT = 7;
const POSSIBLE_LIMIT = 3;

export function calculateScores(profileId, answers = {}, baseAnswers = {}) {
  const types = insuranceTypes[profileId];
  const scores = Object.fromEntries(
    Object.keys(types).map((key) => [key, { key, score: 0, reasons: [], existing: false }])
  );

  for (const question of [...(baseQuestions[profileId] || []), ...(quickQuestions[profileId] || [])]) {
    const answerBag = (baseQuestions[profileId] || []).some((item) => item.id === question.id) ? baseAnswers : answers;
    const values = toArray(answerBag[question.id]);
...
export function calculateScoresLegacy(profileId, answers = {}) {
  const types = insuranceTypes[profileId];
  const scores = Object.fromEntries(
    Object.keys(types).map((key) => [key, { key, score: 0, reasons: [], existing: false }])
  );
